#fileName = 'raw_DA_LMP/%s-da.csv' %formDate
...
zipcutoff = dt.date(2014, 3, 20 ) #this is a date where they start zipping
dates = []
d = start_date
delta = dt.timedelta(days=1)

header = append( 'date', zones )
f1 = open("model_data/ml_DAx.csv", 'a')
f2 = open("model_data/cong_DAx.csv", 'a')
f3 = open("model_data/energy_DAx.csv", 'a')
...
while d<= end_date:
    ...
    print formDate
    if d > zipcutoff:
        url = 'http://www.pjm.com/pub/account/lmpda/%s-da.csv' %formDate 
        rawLMP = pd.read_csv( url )
    else:
        url = 'http://www.pjm.com/pub/account/lmpda/%s-da.zip' %formDate
        r = requests.get(url)
        z = zipfile.ZipFile(StringIO.StringIO(r.content))
        csvFile = '%s-da.csv' %formDate
